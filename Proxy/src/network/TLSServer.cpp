...
mbedtls_ssl_context TLSServer::getSSL() {
	return m_ssl;
}
...
bool TLSServer::recvMsg(std::string &msg, mbedtls_ssl_context ssl) {
...
bool TLSServer::sendMsg(std::string &msg, mbedtls_ssl_context ssl) {
	...
	do {
		ret = mbedtls_ssl_write(&ssl, (const unsigned char *)msg.c_str(), msg.length());
	} while (ret == MBEDTLS_ERR_SSL_WANT_READ || ret == MBEDTLS_ERR_SSL_WANT_WRITE);
